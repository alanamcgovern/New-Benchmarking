...
for(setting_num in 1:4){
  load(file = paste0("/Users/alanamcgovern/Desktop/Research/New_Benchmarking/Simulation_Study/sim_setting_",setting_num,".rda"))
  load(file = paste0("/Users/alanamcgovern/Desktop/Research/New_Benchmarking/Simulation_Study/sim",setting_num,"results_all_admin1.rda"))
  load(file = paste0("/Users/alanamcgovern/Desktop/Research/New_Benchmarking/Simulation_Study/sim",setting_num,"results_all_admin2.rda"))
...
disc_plot <-  admin1.full.res %>% group_by(admin1.ordered,sim_label,pop_nmr) %>% 
  reframe(stan = 1000*mean(stan.disc),bench = 1000*mean(bench.disc),exact = 1000*mean(exact.disc)) %>% 
  ggplot() + 
  ...
  scale_color_manual(name='Model', values=c('bench'=bench_color,'stan'=stan_color,'exact'=exact_color),
                     labels=c('bench'='DABUL','stan'='Standard unit-level (nested)','exact'='Exact DABUL')) +
...
perc_disc_plot <- admin1.full.res %>%
  ...
  ylab('% decrease in discrepancy b/w UL and DABUL') + 
...
mae_plot <-  admin2.full.res %>% group_by(admin2.ordered,sim_label) %>% 
  ...
  pivot_longer(cols=c('stan','bench','exact')) %>% 
  ggplot() + 
  ...
  scale_linetype_discrete(name='Model', labels=c('DABUL','Exact DABUL','Standard unit-level (nested)')) +
...
mae_boxplot <- admin2.full.res %>% group_by(A2, sim_label) %>% 
  reframe(stan = (mean(abs((1000*stan.median)-(1000*pop_nmr)))),
          ...
          exact = (mean(abs((1000*exact.median)-(1000*pop_nmr))))) %>%
  pivot_longer(cols=c('stan','bench','exact')) %>% 
  ggplot() + geom_boxplot(aes(x=sim_label,fill=name,y=value)) + 
...
coefvar_plot <-  admin2.full.res %>% group_by(admin2.ordered,sim_label,pop_nmr) %>% 
 reframe(stan = mean(stan.sd/stan.median),bench = mean(bench.sd/bench.median), exact=mean(exact.sd/exact.median)) %>% 
  pivot_longer(cols=c('stan','bench','exact')) %>% 
  ggplot() + 
  geom_line(aes(x=admin2.ordered,y=value,col=name,lty=name)) +
  scale_x_continuous(name=c('Second administrative area, in order of population NMR')) +
...
coefvar_boxplot <- admin2.full.res %>% group_by(A2, sim_label) %>% 
  # reframe(stan = mean(stan.width), bench= mean(bench.width), exact=mean(exact.width)) %>% 
  reframe(stan = mean(stan.sd/stan.median),bench = mean(bench.sd/bench.median), exact=mean(exact.sd/exact.median)) %>% 
  pivot_longer(cols=c('stan','bench','exact')) %>% 
  ggplot() + geom_boxplot(aes(x=sim_label,fill=name,y=value)) + 
  scale_fill_manual(name='Model',
...
cov_plot <-  admin2.full.res %>% group_by(admin2.ordered,sim_label,pop_nmr) %>% 
  reframe(stan = mean(stan.coverage),bench = mean(bench.coverage), exact=mean(exact.coverage)) %>% 
  pivot_longer(cols=c('stan','bench','exact')) %>% 
  ggplot() + 
  geom_line(aes(x=admin2.ordered,y=value,col=name,lty=name)) +
