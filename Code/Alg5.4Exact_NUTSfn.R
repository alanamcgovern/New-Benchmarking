...
postsamp_Alg5.4Exact_NUTS_MCMC_onechain <- function(alpha_tau = 0.01, U_tau = 1, alpha_phi=0.5, beta_phi=0.5, 
                                                    mu_U = -3.5, sd_U = 3, mu_R = -3.5, sd_R = 3, lambda_d = 1, 
  ...
  for(i in 2:n_iter){
    ...
    #     stop(paste0("Yplus_",area," is about to be set to NA"))
    ...
    for(k in 1:nrow(data)){
      ...
      f <- log_target_cond_5.3_y(domain,data_table[A1==area_id,],Yplus_current[area_id],
      ...
      U <- runif(1)
      if(is.na(min(domain[target_F>U])))
        stop(paste0("Y_",area," is about to be set to NA"))
