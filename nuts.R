...
f <- log_target_5.3a_reg_params_4NUTS
grad_f=grad_log_target_5.3a_4NUTS

max_treedepth = 10
verbose = TRUE
DA=F
kappa = 0.75
t0 = 10# stabilizes initial iterations
gamma = 0.05 #controls shrinkage towards mu
delta = 0.5
theta <- reg_params_current
...
## version in Github (used version is edited from github comment)
# while(s == 1){
#   # choose direction {-1, 1}
#   direction <- sample(c(-1, 1), 1)
#   if(direction == -1){
